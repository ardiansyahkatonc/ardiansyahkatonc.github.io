...
const outDir = 'd:/kat.on/src/components/ui';
...
- Don't use purely for decorative spacing (use margins or layout utilities instead).`
  },
  {
    ...
    astro: `---
interface Props {
  name: string; // The identifier for the icon to render
  class?: string;
...
// Centralized wrapper. 
// Uses a placeholder SVG. Replace with actual icon sets (e.g. Lucide) dynamically based on \`name\`.
---
<svg 
...
components.forEach(comp => {
  const compNameCapitalized = comp.name.charAt(0).toUpperCase() + comp.name.slice(1);
  const compDir = path.join(outDir, comp.name);
  
  if (!fs.existsSync(compDir)) {
    fs.mkdirSync(compDir, { recursive: true });
  }

  fs.writeFileSync(path.join(compDir, \`\${compNameCapitalized}.astro\`), comp.astro);
  fs.writeFileSync(path.join(compDir, 'index.ts'), \`export { default as \${compNameCapitalized} } from './\${compNameCapitalized}.astro';\\n\`);
  fs.writeFileSync(path.join(compDir, 'README.md'), comp.readme);
});
